...
def getXlsxString(sh, i, in_columns_j):
    ...
    for item in in_columns_j.keys() :
        j = in_columns_j[item]
        if item in ('закупка','продажа','цена','цена1','цена2') :
...
#   for i in range(1, sheet.nrows) :                                     # xls
        ...
        try:
            ...
            if (impValues['цена1']=='0' and 
                ...
                if  recOut["закупка"]=="0" and csvFName == "csv_lg_com_tv.csv":
...
def download( cfg ):
    ...
    try:
        ...
        driver.find_element(By.ID, "USER_LOGIN_POPUP").send_keys(login)
